...
export default function StateClaimsPage({ params }: { params: { stateSlug: string } }) {
  const [filters, setFilters] = useState({ state: params.stateSlug, status: '' });
  const [selectedItem, setSelectedItem] = useState<string | null>(null);
  const { data, loading, error, retry, setFilters: updateFilters } = useDashboardFetch(apiClient, { endpoint: 'claims', page: 1, limit: 15, filters });
  ...
  const listItems = (data?.items || []).map((item: any) => ({
    ...
    metadata: [{ label: 'Type', value: item.claimType }, { label: 'Area', value: `${item.areaAcres || 0}ac` }],
  ...
  return (
    <DashboardLayout role="state-officer" title="State" titleHi="">
      <DashboardPageContainer title="State Claims Management">
        ...
        <section className="mb-6">
          ...
          <DashboardFilterBar filters={filters} onFilterChange={(k, v) => setFilters(p => ({ ...p, [k]: v }))} filterOptions={[{ key: 'status', label: 'Status', type: 'select' as const, options: [{ label: 'Approved', value: 'APPROVED' }] }]} onApply={() => updateFilters(filters)} />
        </section>
        <section>
